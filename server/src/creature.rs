...
use protocol::packet::Item;
use protocol::utils::{FlagSet16, FlagSet32};
...
impl Creature {
	pub fn maybe_from(creature_update: &CreatureUpdate) -> Option<Creature> {
		//todo: macro?
			...
			climb_animation_state: creature_update.climb_animation_state?,
	...
	pub fn update(&mut self, packet: &CreatureUpdate) {
		...
		if let Some(it) = packet.climb_animation_state { self.climb_animation_state = it }
	...
	pub fn to_update(&self) -> CreatureUpdate {
		CreatureUpdate {
			...
			climb_animation_state: Some(self.climb_animation_state),
